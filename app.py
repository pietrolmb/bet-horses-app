...
def genera_nuova_corsa():
    num = random.randint(data["settings"]["min_horses"], data["settings"]["max_horses"])
    horses = []
    ...
    weights = []
    for i in range(num):
        if i < num_fav: weights.append(random.uniform(100, 180)) # Super favoriti
        else: weights.append(random.uniform(10, 50)) # Outsider
    
    random.shuffle(weights)
    tot_w = sum(weights)
    p_vittoria = [w / tot_w for w in weights]
    
    # Lavagna (Margine Banco) al 35% (Molto alto!)
    house_edge = 1.35 
    ...
    for i in range(num):
        ...
        horses.append({
            ...
            "quota_v": q_v, "quota_p": q_p, "quota_u": max(1.10, round(5/q_v, 2)),
...
def handle_wallet(req):
    ...
    if u not in data["users"]: 
        data["users"][u] = {"wallet": 0.0, "password": req.get('password','1234'), "tot_dep":0.0, "tot_vin":0.0, "tot_per":0.0}
    data["users"][u]["wallet"] = round(data["users"][u]["wallet"] + amt, 2)
    if amt > 0: data["users"][u]["tot_dep"] += amt 
    socketio.emit('update_data', data)

@socketio.on('admin_update_settings')
def update_settings(req):
    for k in req:
        if k in data["settings"]: data["settings"][k] = type(data["settings"][k])(req[k])
    socketio.emit('update_data', data)
...
def login_check(req):
    u, p = req.get('user','').strip(), req.get('password','').strip()
    if u in data["users"] and data["users"][u]["password"] == p:
        if u not in data["online_users"]: data["online_users"].append(u)
        emit('login_success', {"user": u})
        socketio.emit('update_data', data)
    else: emit('login_error', "Credenziali errate!")
...
def handle_bet(bet_data):
    ...
    if u not in data["users"] or data["users"][u]["wallet"] < amt:
        emit('login_error', "Saldo insufficiente!"); return
    ...
    if race["status"] in ["waiting", "countdown"]:
        ...
        race["bets"].append({
            "user": u, "type": tipo, "dettaglio": bet_data['dettaglio'], "amount": amt, "quota": float(bet_data['quota']),
            "h1": bet_data.get('h1'), "h2": bet_data.get('h2'), "h3": bet_data.get('h3'), "ordine": bet_data.get('ordine', True), "esito": "In Attesa"
        })
...
def start_race():
    race = data["current_race"]
    race["status"] = "racing"
    socketio.emit('update_data', data) # Chiude subito le scommesse
    socketio.emit('race_start')
    
    steps = 250 
    # NUOVA FISICA: Ogni cavallo ha una "giornata" diversa
    forma_segreta = {h["id"]: random.uniform(0.6, 1.4) for h in race["horses"]}
    
    raw_progress = {h["id"]: [0]*steps for h in race["horses"]}
    for h in race["horses"]:
        for step in range(steps):
            # La probabilità di vittoria conta molto meno, domina il caos e la forma
    ...
    for step in range(steps):
        for h in race["horses"]: current_pos[h["id"]] += (raw_progress[h["id"]][step] * scale)
        socketio.emit('race_update', current_pos)
        socketio.sleep(0.12)

    classifica = sorted(race["horses"], key=lambda h: current_pos[h["id"]], reverse=True)
    id_1, id_2, id_3, id_ult = classifica[0]["id"], classifica[1]["id"], classifica[2]["id"], classifica[-1]["id"]
    ...
    res = {
        "gara_num": len(data["history"]) + 1, "primo_id": id_1, "primo_nome": classifica[0]["nome"],
        "primo": f"N°{id_1} - {classifica[0]['nome']}", "q_primo": classifica[0]["quota_v"],
        "secondo": f"N°{id_2} - {classifica[1]['nome']}", "terzo": f"N°{id_3} - {classifica[2]['nome']}", 
        "ultimo": f"N°{id_ult} - {classifica[-1]['nome']}", "q_ultimo": classifica[-1]["quota_u"],
        "vincitori": [{"user": v["user"], "vincita": v["vincita"], "dettaglio": v["dettaglio"]} for v in [] if False], # Placeholder
        "scommesse": race["bets"].copy() 
    }
    # Fix per la lista vincitori nel risultato
    res["vincitori"] = [{"user": b["user"], "vincita": round(b["amount"]*b["quota"],2), "dettaglio": b["dettaglio"]} for b in race["bets"] if b["esito"] == "Vinta"]
    data["history"].append(res)
    socketio.emit('race_finished', res)
    socketio.sleep(15) 
    race["status"], race["horses"], race["bets"], race["timer"] = "waiting", genera_nuova_corsa(), [], 0
    socketio.emit('update_data', data)
